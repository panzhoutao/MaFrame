...
import com.datepicker.bizs.CalendarBiz;
import com.datepicker.entities.BGCircle;
        ...
        for (int i = 0; i < mRegion.length; i++) {
            for (int j = 0; j < mRegion[i].length; j++) {
                ...
                if (region.contains(x, y)) {
                    ...
                    final String date = df4.format(currentYear) + "/" + df2.format(currentMonth) + "/" + df2.format(Integer.parseInt(mCalendarBiz.getGregorianCreated().get(index)[i][j]));
