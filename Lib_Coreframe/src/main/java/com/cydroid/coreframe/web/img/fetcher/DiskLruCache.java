...
public class DiskLruCache {
	...
	private final int maxCacheItemSize = 500; // 64 item default
	 ...
	 */
	public Bitmap get(String key,ImageCache cache) {
		synchronized (mLinkedHashMap) {
			...
			if (file != null) {
				...
				Bitmap bitmap = null;
				try{
					bitmap=PictureUtil.getimage(file, 1200, 1200, 2*1024*1024);
				}catch(OutOfMemoryError e){

					e.printStackTrace();
					cache.clearMemorCaches();
...
//						BitmapFactory.decodeFile(file);
				...
				if (new File(existingFile).exists()) {
					...
					Bitmap bitmap = null;
					try{
						bitmap=PictureUtil.getimage(existingFile, 1300, 1300, 2*1024*1024);
					}catch(OutOfMemoryError e){
